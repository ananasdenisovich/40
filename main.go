...
func RegisterUser(user User) error {
	...
	err = sendConfirmationEmail(user.Email, user.ConfirmToken)
	if err != nil {
		return err
	}
...
func getRandomEmail() string {
	adjectives := []string{"happy", "sunny", "bright", "shiny", "joyful", "lucky", "cheery", "gleeful", "vivid", "breezy"}
	nouns := []string{"bird", "flower", "butterfly", "sunshine", "rainbow", "star", "ocean", "mountain", "cloud", "tree"}
	extensions := []string{"com", "net", "org"}

	randAdjective := adjectives[rand.Intn(len(adjectives))]
	randNoun := nouns[rand.Intn(len(nouns))]
	randExtension := extensions[rand.Intn(len(extensions))]

	return fmt.Sprintf("%s.%s@%s.%s", randAdjective, randNoun, randNoun, randExtension)
}
